...
"""
Unit tests for vcenter_vm_guest_networking_info module.
...
networking configuration as a single object.
"""
...
from ansible_collections.vmware.vmware_rest.plugins.modules import (
    vcenter_vm_guest_networking_info as module_under_test,
)
...
# A realistic guest networking response for /vcenter/vm/{vm}/guest/networking.
GUEST_NETWORKING = {
    "dns_values": {
...
# ============================================================================
# Test GET Operations (per-VM guest networking)
# ============================================================================


def test_get_networking_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the networking config for a VM that reports one."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, GUEST_NETWORKING
    )
    ...
    assert result["value"] == GUEST_NETWORKING
    # "info" is always a list of dicts.
    assert result["info"] == [GUEST_NETWORKING]
    # The networking payload carries no MOID attribute, so no id is set.
    assert "id" not in result


def test_get_networking_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the networking config when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting the networking config in check mode still queries the API."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args({**module_args, "vm": "vm-42"})
        ...
        mock_client.get.return_value = _response(200, GUEST_NETWORKING)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        assert call_args[0][0] == "/vcenter/vm/vm-42/guest/networking"
...
class TestModuleConstants:
    ...
    def test_item_endpoint(self):
        ...
        assert module_under_test.ITEM_ENDPOINT == "/vcenter/vm/{vm}/guest/networking"

    def test_get_operation_config(self):
        """Test that the GET operation config targets the item endpoint."""
        assert (
            module_under_test.GET_OPERATION.uri == "/vcenter/vm/{vm}/guest/networking"
        )
