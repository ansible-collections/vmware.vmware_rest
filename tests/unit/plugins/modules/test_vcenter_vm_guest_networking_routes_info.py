...
"""
Unit tests for vcenter_vm_guest_networking_routes_info module.
...
(keyed on the required C(vm) path parameter) whose GET returns a list of
guest network route records.
"""
...
from ansible_collections.vmware.vmware_rest.plugins.modules import (
    vcenter_vm_guest_networking_routes_info as module_under_test,
)
...
# Realistic guest route records for /vcenter/vm/{vm}/guest/networking/routes.
ROUTE_DEFAULT = {
    "network": "0.0.0.0",
    "prefix_length": 0,
...
GUEST_ROUTES = [ROUTE_DEFAULT, ROUTE_LOCAL]
...
# ============================================================================
# Test GET Operations (per-VM guest network routes)
# ============================================================================


def test_get_routes_multiple(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting routes when the guest reports several."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, GUEST_ROUTES
    )
    ...
    # Multiple resources keep list shape in both "value" and "info".
    assert result["info"] == GUEST_ROUTES
    assert result["value"] == GUEST_ROUTES
    # Multiple resources have no single id.
    assert "id" not in result


def test_get_routes_single(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting routes when the guest reports exactly one."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, [ROUTE_DEFAULT]
    )
    ...
    mock_module.exit_json.assert_called_once()
    result = exc.value.kwargs
    # A single result collapses "value" to the lone dict, but "info" stays a list.
    assert result["info"] == [ROUTE_DEFAULT]
    assert result["value"] == ROUTE_DEFAULT
    # A route record carries no MOID attribute, so no id is set.
    assert "id" not in result
...
def test_get_routes_empty(
    ...
    """Test getting routes when the guest reports none."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(patch_ansible_module, mock_client, module_args, 200, [])
...
def test_get_routes_not_found(
    ...
    """Test getting routes when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting routes in check mode still queries the API."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args({**module_args, "vm": "vm-42"})
        ...
        mock_client.get.return_value = _response(200, GUEST_ROUTES)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        patch_create_client.return_value = mock_client
        _run_module(patch_ansible_module, mock_client, module_args, 200, GUEST_ROUTES)
        ...
        assert call_args[0][0] == "/vcenter/vm/vm-42/guest/networking/routes"
