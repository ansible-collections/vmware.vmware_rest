...
"""
Unit tests for appliance_system_time_info module.
...
returns the appliance's current system time as a structured object.
"""
...
from ansible_collections.vmware.vmware_rest.plugins.modules import (
    appliance_system_time_info as module_under_test,
)
...
SYSTEM_TIME = {
    "seconds_since_epoch": 1406837912.0,
    "date": "Thu 07-31-2014",
    "time": "18:18:32",
    "timezone": "UTC",
}
...
# ============================================================================
# Test GET Operations (Singleton returning a system time struct)
# ============================================================================
...
def test_get_system_time(
    ...
    """Test getting the appliance system time."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, SYSTEM_TIME
    )
    ...
    assert result["value"] == SYSTEM_TIME
    assert result["info"] == [SYSTEM_TIME]
    assert result["value"]["timezone"] == "UTC"
    # A time struct has no MOID attribute, so no id should be reported.
    assert "id" not in result


def test_get_system_time_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the system time when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting the system time in check mode."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args(module_args)
        ...
        mock_client.get.return_value = _response(200, SYSTEM_TIME)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        patch_create_client.return_value = mock_client
        _run_module(patch_ansible_module, mock_client, module_args, 200, SYSTEM_TIME)
        ...
        assert call_args[0][0] == "/appliance/system/time"
...
class TestModuleConstants:
    ...
    def test_item_endpoint(self):
        ...
        assert module_under_test.ITEM_ENDPOINT == "/appliance/system/time"

    def test_get_operation_config(self):
        """Test that the GET operation config targets the item endpoint."""
        assert module_under_test.GET_OPERATION.uri == "/appliance/system/time"
