...
"""
Unit tests for appliance_networking_interfaces_ipv4_info module.

Tests validate the Info module behavior using the OperationConfig-based
architecture with mocked HTTP clients.

``appliance_networking_interfaces_ipv4_info`` is a read-only module that returns
the IPv4 configuration of a single named appliance interface. It performs a GET
against ``/appliance/networking/interfaces/{interface_name}/ipv4`` (there is no
...
def test_get_ipv4_info(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test retrieving the IPv4 configuration of an interface."""
    ...
    # The IPv4 config has no recognizable MOID attribute, so no top-level id.
    assert "id" not in result


def test_get_ipv4_info_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        """Test that GET uses the interface's IPv4 item path with no query."""
        patch_create_client.return_value = mock_client
        module_args.update({"interface_name": "nic0"})
