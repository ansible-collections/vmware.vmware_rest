...
"""
Unit tests for vcenter_vm_guest_power_info module.
...
(keyed on the required C(vm) path parameter) that returns the guest OS
power state as a single object.
"""
...
# A realistic guest power response for /vcenter/vm/{vm}/guest/power.
GUEST_POWER = {
    "state": "RUNNING",
...
# ============================================================================
# Test GET Operations (per-VM guest power state)
# ============================================================================
...
def test_get_power_found(
    ...
    """Test getting the guest power state for a VM that reports one."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, GUEST_POWER
    )
    ...
    assert result["value"] == GUEST_POWER
    # "info" is always a list of dicts.
    assert result["info"] == [GUEST_POWER]
    # The power payload carries no MOID attribute, so no id is set.
    assert "id" not in result


@pytest.mark.parametrize(
    "power_state", ["RUNNING", "POWERED_OFF", "SUSPENDED", "NOT_RUNNING"]
)
def test_get_power_states(
    ...
    """Test getting each reported guest power state value."""
    patch_create_client.return_value = mock_client
    payload = {"state": power_state, "operations_ready": False}
...
def test_get_power_not_found(
    ...
    """Test getting the guest power state when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting the guest power state in check mode still queries the API."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args({**module_args, "vm": "vm-42"})
        ...
        mock_client.get.return_value = _response(200, GUEST_POWER)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        patch_create_client.return_value = mock_client
        _run_module(patch_ansible_module, mock_client, module_args, 200, GUEST_POWER)
        ...
        assert call_args[0][0] == "/vcenter/vm/vm-42/guest/power"
...
class TestModuleConstants:
    ...
    def test_item_endpoint(self):
        ...
        assert module_under_test.ITEM_ENDPOINT == "/vcenter/vm/{vm}/guest/power"

    def test_get_operation_config(self):
        """Test that the GET operation config targets the item endpoint."""
        assert module_under_test.GET_OPERATION.uri == "/vcenter/vm/{vm}/guest/power"
