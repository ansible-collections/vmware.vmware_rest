...
"""
Unit tests for appliance_system_version_info module.
...
returns the appliance's version information as a structured object.
"""
...
from ansible_collections.vmware.vmware_rest.plugins.modules import (
    appliance_system_version_info as module_under_test,
)
...
SYSTEM_VERSION = {
    ...
    "summary": "Patch for VMware vCenter Server 9.1.0",
...
# ============================================================================
# Test GET Operations (Singleton returning a version struct)
# ============================================================================
...
def test_get_system_version(
    ...
    """Test getting the appliance version information."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, SYSTEM_VERSION
    )
    ...
    assert result["value"] == SYSTEM_VERSION
    assert result["info"] == [SYSTEM_VERSION]
    assert result["value"]["version"] == "9.1.0.10000"
    assert result["value"]["product"] == "VMware vCenter Server"
    # A version struct has no MOID attribute, so no id should be reported.
    assert "id" not in result


def test_get_system_version_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the version when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting the version information in check mode."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args(module_args)
        ...
        mock_client.get.return_value = _response(200, SYSTEM_VERSION)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        patch_create_client.return_value = mock_client
        _run_module(patch_ansible_module, mock_client, module_args, 200, SYSTEM_VERSION)
        ...
        assert call_args[0][0] == "/appliance/system/version"
...
class TestModuleConstants:
    ...
    def test_item_endpoint(self):
        ...
        assert module_under_test.ITEM_ENDPOINT == "/appliance/system/version"

    def test_get_operation_config(self):
        """Test that the GET operation config targets the item endpoint."""
        assert module_under_test.GET_OPERATION.uri == "/appliance/system/version"
