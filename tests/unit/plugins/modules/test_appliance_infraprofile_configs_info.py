...
# GET /appliance/infraprofile/configs — list, 200 array of profile summaries
PROFILE_LIST = [
...
def test_list_success(mock_create_client, mock_ansible_module, mock_client):
    ...
    mock_client.get.return_value = _response(200, PROFILE_LIST)

    with pytest.raises(AnsibleExitJson) as exc:
        module_under_test.main()

    mock_client.get.assert_called_once_with(
        "/appliance/infraprofile/configs", query=None
    )
...
@patch.object(module_under_test, "AnsibleModule")
@patch.object(module_under_test.VmwareRestInfoModule, "_create_client")
def test_list_not_found(mock_create_client, mock_ansible_module, mock_client):
    mock_create_client.return_value = mock_client
    mock_module = MagicMock()
    mock_ansible_module.return_value = mock_module
    mock_module.params = set_module_args({})
