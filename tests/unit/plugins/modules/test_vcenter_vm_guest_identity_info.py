...
"""
Unit tests for vcenter_vm_guest_identity_info module.
...
(keyed on the required C(vm) path parameter) that returns a single guest
identity object.
"""
...
from ansible_collections.vmware.vmware_rest.plugins.modules import (
    vcenter_vm_guest_identity_info as module_under_test,
)
...
# A realistic guest identity response for /vcenter/vm/{vm}/guest/identity.
GUEST_IDENTITY = {
    "name": "WINDOWS_SERVER_2019_64",
    "family": "WINDOWS",
    "full_name": {
        "id": "v.guestOS.windows.serverstandard",
        "default_message": "Microsoft Windows Server 2019 (64-bit)",
        "args": [],
    },
    "host_name": "win-guest.example.com",
...
# ============================================================================
# Test GET Operations (per-VM guest identity)
# ============================================================================


def test_get_guest_identity_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting guest identity for a VM that reports one."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, GUEST_IDENTITY
    )
    ...
    assert result["value"] == GUEST_IDENTITY
    # "info" is always a list of dicts.
    assert result["info"] == [GUEST_IDENTITY]
    # The guest identity payload carries no MOID attribute, so no id is set.
    assert "id" not in result


def test_get_guest_identity_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting guest identity when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting guest identity in check mode still queries the API."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args({**module_args, "vm": "vm-42"})
        ...
        mock_client.get.return_value = _response(200, GUEST_IDENTITY)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        patch_create_client.return_value = mock_client
        _run_module(patch_ansible_module, mock_client, module_args, 200, GUEST_IDENTITY)
        ...
        assert call_args[0][0] == "/vcenter/vm/vm-42/guest/identity"
...
class TestModuleConstants:
    ...
    def test_item_endpoint(self):
        ...
        assert module_under_test.ITEM_ENDPOINT == "/vcenter/vm/{vm}/guest/identity"

    def test_get_operation_config(self):
        """Test that the GET operation config targets the item endpoint."""
        assert module_under_test.GET_OPERATION.uri == "/vcenter/vm/{vm}/guest/identity"
