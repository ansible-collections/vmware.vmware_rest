...
"""
Unit tests for content_subscribedlibrary_info module.
...
SAMPLE_LIBRARY = {
    ...
    "subscription_info": {
        ...
        "on_demand": False,
        "subscription_url": "https://vcenter.test:443/cls/vcsp/lib/abc/lib.json",
    },
}
...
def test_get_library_by_id(
    ...
    """Test getting a specific subscribed content library by its library_id."""
    patch_create_client.return_value = mock_client
    mock_module = MagicMock()
    patch_ansible_module.return_value = mock_module

    module_args.update({"library_id": "lib-2001"})
    ...
    assert result["id"] == "lib-2001"

    # GET should target the item endpoint with the library_id substituted in.
    mock_client.get.assert_called_once()
    assert mock_client.get.call_args[0][0] == "/content/subscribed-library/lib-2001"


def test_get_library_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting a subscribed content library that doesn't exist (404)."""
    patch_create_client.return_value = mock_client
    mock_module = MagicMock()
    patch_ansible_module.return_value = mock_module
...
def test_list_all_libraries(
    ...
    """Test listing all subscribed libraries when no library_id is provided.
    ...
    list_response = ["lib-2001", "lib-2002"]
    detail_1 = {"id": "lib-2001", "name": "library-a", "type": "SUBSCRIBED"}
    detail_2 = {"id": "lib-2002", "name": "library-b", "type": "SUBSCRIBED"}
    ...
    assert result["info"][0]["library_id"] == "lib-2001"
    assert result["info"][0]["name"] == "library-a"
    assert result["info"][1]["library_id"] == "lib-2002"
    assert result["info"][1]["name"] == "library-b"

    # First call lists the collection, the remaining calls fetch each detail.
    assert mock_client.get.call_count == 3
    assert mock_client.get.call_args_list[0][0][0] == "/content/subscribed-library"
    assert (
        mock_client.get.call_args_list[1][0][0]
        == "/content/subscribed-library/lib-2001"
    )
    assert (
        mock_client.get.call_args_list[2][0][0]
        == "/content/subscribed-library/lib-2002"
    )
...
def test_list_libraries_summaries_used_as_is(
    ...
    list_response = [
        {"id": "lib-2001", "name": "library-a"},
        {"id": "lib-2002", "name": "library-b"},
    ]
    ...
    mock_client.get.assert_called_once()
    assert mock_client.get.call_args[0][0] == "/content/subscribed-library"


def test_list_libraries_empty(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test listing subscribed libraries when none exist."""
    patch_create_client.return_value = mock_client
    mock_module = MagicMock()
    patch_ansible_module.return_value = mock_module
...
class TestCheckMode:
    ...
    def test_list_check_mode(
        ...
        mock_client.get.side_effect = [
            _response(200, ["lib-2001"]),
            _response(200, {"id": "lib-2001", "name": "library-a"}),
        ]
...
class TestModuleConstants:
    ...
    def test_list_endpoint(self):
        ...
        assert module_under_test.LIST_ENDPOINT == "/content/subscribed-library"

    def test_item_endpoint(self):
        """Test that the item API endpoint is correct."""
        assert (
            module_under_test.ITEM_ENDPOINT
            == "/content/subscribed-library/{library_id}"
        )
    ...
    def test_list_operation_config(self):
        ...
        assert module_under_test.LIST_OPERATION.uri == "/content/subscribed-library"
