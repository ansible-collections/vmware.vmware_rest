...
"""
Unit tests for vcenter_vm_guest_localfilesystem_info module.
...
(keyed on the required C(vm) path parameter) that returns the guest local
filesystem details as a map of mount points to their usage info.
"""
...
from ansible_collections.vmware.vmware_rest.plugins.modules import (
    vcenter_vm_guest_localfilesystem_info as module_under_test,
)
...
# A realistic local filesystem response for
# /vcenter/vm/{vm}/guest/local-filesystem. The payload is a map of guest
# mount points to their capacity/free-space usage.
LOCAL_FILESYSTEM = {
    "C:\\": {
...
# ============================================================================
# Test GET Operations (per-VM guest local filesystem)
# ============================================================================


def test_get_local_filesystem_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the local filesystem info for a VM that reports one."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, LOCAL_FILESYSTEM
    )
    ...
    assert result["value"] == LOCAL_FILESYSTEM
    # "info" is always a list of dicts.
    assert result["info"] == [LOCAL_FILESYSTEM]
    # The filesystem map carries no MOID attribute, so no id is set.
    assert "id" not in result


def test_get_local_filesystem_empty(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the local filesystem when the guest reports no mounts."""
    patch_create_client.return_value = mock_client
    ...
    mock_module.exit_json.assert_called_once()
    result = exc.value.kwargs
    # An empty dict is falsy, so it is normalized to no results.
    assert result["info"] == []
    assert result["value"] == {}
    assert "id" not in result


def test_get_local_filesystem_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the local filesystem when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting the local filesystem in check mode still queries the API."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args({**module_args, "vm": "vm-42"})
        ...
        mock_client.get.return_value = _response(200, LOCAL_FILESYSTEM)
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        assert call_args[0][0] == "/vcenter/vm/vm-42/guest/local-filesystem"
...
class TestModuleConstants:
    ...
    def test_item_endpoint(self):
        ...
        assert (
            module_under_test.ITEM_ENDPOINT == "/vcenter/vm/{vm}/guest/local-filesystem"
        )
