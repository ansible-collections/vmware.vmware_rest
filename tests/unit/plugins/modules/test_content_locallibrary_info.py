...
"""
Unit tests for content_locallibrary_info module.
...
from ansible_collections.vmware.vmware_rest.plugins.modules import (
    content_locallibrary_info as module_under_test,
)
...
def test_get_library_by_id(
    ...
    """Test getting a specific local content library by its library_id."""
    ...
    assert mock_client.get.call_args[0][0] == "/content/local-library/lib-1001"


def test_get_library_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting a local content library that doesn't exist (404)."""
    patch_create_client.return_value = mock_client
    mock_module = MagicMock()
    patch_ansible_module.return_value = mock_module
...
def test_list_all_libraries(
    ...
    """Test listing all local libraries when no library_id is provided.
    ...
    detail_2 = {"id": "lib-1002", "name": "library-b", "type": "LOCAL"}
    ...
    assert mock_client.get.call_args_list[0][0][0] == "/content/local-library"
    assert mock_client.get.call_args_list[1][0][0] == "/content/local-library/lib-1001"
    assert mock_client.get.call_args_list[2][0][0] == "/content/local-library/lib-1002"
...
def test_list_libraries_summaries_used_as_is(
    ...
    mock_client.get.assert_called_once()
    assert mock_client.get.call_args[0][0] == "/content/local-library"


def test_list_libraries_empty(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test listing local libraries when none exist."""
    patch_create_client.return_value = mock_client
    mock_module = MagicMock()
    patch_ansible_module.return_value = mock_module
...
class TestModuleConstants:
    ...
    def test_list_endpoint(self):
        ...
        assert module_under_test.LIST_ENDPOINT == "/content/local-library"

    def test_item_endpoint(self):
        """Test that the item API endpoint is correct."""
        assert module_under_test.ITEM_ENDPOINT == "/content/local-library/{library_id}"

    def test_get_operation_config(self):
        """Test that the GET operation config targets the item endpoint."""
        assert (
            module_under_test.GET_OPERATION.uri == "/content/local-library/{library_id}"
        )
    ...
    def test_list_operation_config(self):
        ...
        assert module_under_test.LIST_OPERATION.uri == "/content/local-library"
