...
"""
Unit tests for vcenter_vm_tools_installer_info module.
...
/vcenter/vm/{vm}/tools/installer returns the VMware Tools installer state of the
virtual machine as a single object.
"""
...
# ============================================================================
# Test GET Operations (VMware Tools Installer State)
# ============================================================================


def test_get_installer_state_disconnected(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the installer state when the installer media is not mounted."""
    ...
    # value mirrors the single installer state object.
    assert result["value"] == get_response
    assert result["value"]["is_connected"] is False
    # The installer state carries no MOID attribute, so no id is set.
    assert "id" not in result


def test_get_installer_state_connected(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the installer state when the installer media is mounted."""
    patch_create_client.return_value = mock_client
    mock_module = MagicMock()
    patch_ansible_module.return_value = mock_module
...
def test_get_installer_state_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the installer state for a VM that doesn't exist."""
    patch_create_client.return_value = mock_client
    mock_module = MagicMock()
    patch_ansible_module.return_value = mock_module
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting the installer state in check mode (should execute normally)."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
