...
"""
Unit tests for vcenter_vm_storage_policy_compliance_info module.
...
(keyed on the required C(vm) path parameter) that returns the cached storage
policy compliance status as a single object.
"""
...
# A realistic response for /vcenter/vm/{vm}/storage/policy/compliance.
COMPLIANCE_INFO = {
...
# ============================================================================
# Test GET Operations (per-VM compliance status)
# ============================================================================


def test_get_compliance_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the compliance status for a VM that reports one."""
    patch_create_client.return_value = mock_client
    mock_module = _run_module(
        patch_ansible_module, mock_client, module_args, 200, COMPLIANCE_INFO
    )
    ...
    assert result["info"] == [COMPLIANCE_INFO]
    # The compliance payload carries no MOID attribute, so no id is set.
    assert "id" not in result


@pytest.mark.parametrize(
    "overall",
    ["COMPLIANT", "NON_COMPLIANT", "UNKNOWN", "NOT_APPLICABLE", "OUT_OF_DATE"],
)
def test_get_compliance_states(
    patch_create_client,
    ...
    """Test getting each reported overall compliance status value."""
    patch_create_client.return_value = mock_client
    payload = {"overall_compliance": overall, "disks": {}}
...
def test_get_compliance_not_found(
    patch_create_client, patch_ansible_module, mock_client, module_args
):
    """Test getting the compliance status when the endpoint returns 404."""
    patch_create_client.return_value = mock_client
...
class TestCheckMode:
    ...
    def test_get_check_mode(
        ...
        """Test getting the compliance status in check mode still queries the API."""
        patch_create_client.return_value = mock_client
        mock_module = MagicMock()
        patch_ansible_module.return_value = mock_module
        mock_module.params = set_module_args({**module_args, "vm": "vm-42"})
...
class TestAPICallPath:
    ...
    def test_get_api_path(
        ...
        assert call_args[0][0] == "/vcenter/vm/vm-42/storage/policy/compliance"
