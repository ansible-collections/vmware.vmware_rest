...
extends_documentation_fragment:
  ## This section lists other YAML doc fragments that will be used to extend the documentation for the module.
  ## The fragments are loaded first, and then the module documentation is loaded.
  ## This fragment contains the connection parameters for the module (vcenter_hostname, session_timeout, etc.)
...
options:
  ...
  <OPTION_NAME>:
    description:
      ...
      - When describing resource identifiers from the API spec, use "identifier (MOID)" (e.g. "Must be an identifier (MOID) for a C(ResourcePool) resource.").
    ...
    suboptions: A dictionary of suboptions for the option, if the option is a dictionary or the option is a list of element dictionaries. Otherwise, this can be omitted. In the argument spec, this corresponds to the 'options' key.
...
## For info modules, the return dictionary should have a value key, which should be a list of dictionaries, each containing the information about a single resource.
## For crud modules, the return dictionary should utilize the following keys:
## - changed: True if the resource was changed, False otherwise. (This return does not need to be documented in the RETURN section.)
## - value: Any options or attributes that were changed by the operation. Only returned on update, not on create or delete.
## - id: The MOID of the resource, if available.
## Example for a crud module:
id:
  description:
    - The MOID of the resource, if available.
  returned: On success
  sample: "1234567890"
  type: str
...
value:
  description:
    - Any options or attributes that were changed by the operation. Only returned on update, not on create or delete.
  returned: On update
  sample: {
    "some_attribute": {
      "before": "old-value",
