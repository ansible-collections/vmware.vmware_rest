...
short_description: Handle resource of type appliance_localaccounts_info
description: Handle resource of type appliance_localaccounts_info
options:
  username:
    description:
...
async def main():
    required_if = list([["state", "get", ["username"], True],])
...
def build_url(params):
    if params.get("username"):
        ...
        return (
            ("https://{vcenter_hostname}" "/rest/appliance/local-accounts/").format(
                **params
            )
            + params["username"]
...
async def entry_point(module, session):
    ...
    async with session.get(url) as resp:
        ...
        if module.params.get("username"):
            ...
            try:
                if (
                    isinstance(_json["value"][0]["username"], str)
