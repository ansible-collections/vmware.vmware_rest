...
short_description: Handle resource of type appliance_monitoring_info
description: Handle resource of type appliance_monitoring_info
options:
  stat_id:
    description:
...
async def main():
    required_if = list([["state", "get", ["stat_id"], True],])
...
def build_url(params):
    if params.get("stat_id"):
        ...
        return (
            ("https://{vcenter_hostname}" "/rest/appliance/monitoring/").format(
                **params
            )
            + params["stat_id"]
    ...
    return ("https://{vcenter_hostname}" "/rest/appliance/monitoring").format(
        **params
    ) + gen_args(params, _in_query_parameters)
...
async def entry_point(module, session):
    ...
    async with session.get(url) as resp:
        ...
        if module.params.get("stat_id"):
            ...
            try:
                if (
                    isinstance(_json["value"][0]["stat_id"], str)
