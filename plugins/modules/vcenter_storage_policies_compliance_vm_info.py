...
options:
  filter.status:
    description:
    - Compliance Status that a virtual machine must have to match the filter. Atleast
      one status must be specified.
    type: list
  filter.vms:
    description:
...
- python >= 3.6
"""
IN_QUERY_PARAMETER = ["filter.status", "filter.vms"]
...
def prepare_argument_spec():
    ...
    argument_spec["filter.status"] = {"type": "list", "operationIds": ["list"]}
...
def url(params):
    return "https://{vcenter_hostname}/rest/vcenter/storage/policies/compliance/vm".format(
        **params
    ) + gen_args(
