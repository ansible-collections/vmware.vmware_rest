...
short_description: Handle resource of type appliance_networking_proxy
description: Handle resource of type appliance_networking_proxy
options:
  action:
    choices:
    - test
    description:
    - action=test Required with I(state=['test'])
    type: str
  config:
    description:
    - Proxy configuration to test. Required with I(state=['set', 'test'])
...
def prepare_argument_spec():
    ...
    argument_spec["action"] = {"type": "str", "choices": ["test"]}
...
async def main():
    required_if = list(
        [
            ["state", "test", ["action", "config", "host", "protocol"], True],
            ["state", "set", ["config", "protocol"], True],
            ["state", "absent", ["protocol"], True],
        ]
    )
...
def build_url(params):
    return ("https://{vcenter_hostname}" "/rest/appliance/networking/proxy").format(
        **params
    )
...
async def _delete(params, session):
    ...
    subdevice_type = get_subdevice_type("/rest/appliance/networking/proxy/{protocol}")
    if subdevice_type and not params[subdevice_type]:
        _json = await exists(params, session, build_url(params))
        if _json:
            params[subdevice_type] = _json["id"]
    _url = (
        "https://{vcenter_hostname}" "/rest/appliance/networking/proxy/{protocol}"
