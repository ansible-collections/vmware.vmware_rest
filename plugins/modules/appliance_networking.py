...
short_description: Handle resource of type appliance_networking
description: Handle resource of type appliance_networking
options:
  SSO_password:
    description:
  ...
  action:
    ...
    - reset
    description:
    - action=reset Required with I(state=['reset'])
    type: str
  dns:
    description:
    - DNS Configuration to set for the machine
    - 'Valid attributes are:'
    - ' - C(mode) (str): {@name DNSServerMode} Describes DNS Server source (DHCP,static)'
    - '   - Accepted values:'
    - '     - dhcp'
    - '     - is_static'
    - ' - C(servers) (list): List of the currently used DNS servers.'
    type: dict
  hostname:
    description:
  ...
  ipv4:
    ...
    - 'Valid attributes are:'
    - ' - C(address) (str): The IPv4 address, for example, "10.20.80.191".'
    ...
    - ' - C(mode) (str): The {@name Mode} defines different IPv4 address assignment
      modes.'
    - '   - Accepted values:'
    - '     - DHCP'
    - '     - STATIC'
  ...
  ipv6:
    ...
    - 'Valid attributes are:'
    - ' - C(addresses) (list): The list of addresses to be statically assigned.'
    ...
    - ' - C(dhcp) (bool): An address will be assigned by a DHCP server.'
    type: dict
  ipv6_enabled:
    description:
    - IPv6 Enabled or not
...
PAYLOAD_FORMAT = {
    "get": {"query": {}, "body": {}, "path": {}},
    "update": {"query": {}, "body": {"ipv6_enabled": "spec/ipv6_enabled"}, "path": {}},
    "reset": {"query": {"action": "action"}, "body": {}, "path": {}},
    "change": {
        "query": {},
        "body": {
            "SSO_password": "spec/SSO_password",
            "SSO_user": "spec/SSO_user",
            "dns": "spec/dns",
...
def prepare_argument_spec():
    ...
    argument_spec["SSO_password"] = {"type": "str"}
    argument_spec["SSO_user"] = {"type": "str"}
    argument_spec["action"] = {"type": "str", "choices": ["reset"]}
...
async def main():
    required_if = list(
        [
            ["state", "change", ["SSO_password", "SSO_user", "hostname"], True],
            ["state", "reset", ["action"], True],
        ]
    )
...
def build_url(params):
    return ("https://{vcenter_hostname}" "/rest/appliance/networking").format(**params)
...
async def _reset(params, session):
    ...
    subdevice_type = get_subdevice_type("/rest/appliance/networking")
    if subdevice_type and not params[subdevice_type]:
        _json = await exists(params, session, build_url(params))
        if _json:
            params[subdevice_type] = _json["id"]
    _url = ("https://{vcenter_hostname}" "/rest/appliance/networking").format(
        **params
...
async def _update(params, session):
    ...
    _url = ("https://{vcenter_hostname}" "/rest/appliance/networking").format(**params)
