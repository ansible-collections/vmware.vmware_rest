...
short_description: Handle resource of type appliance_networking_interfaces_info
description: Handle resource of type appliance_networking_interfaces_info
options:
  interface_name:
    description:
...
async def main():
    required_if = list([["state", "get", ["interface_name"], True],])
...
def build_url(params):
    if params.get("interface_name"):
        ...
        return (
            (
                "https://{vcenter_hostname}" "/rest/appliance/networking/interfaces/"
            ).format(**params)
...
async def entry_point(module, session):
    ...
    async with session.get(url) as resp:
        ...
        if module.params.get("interface_name"):
            ...
            try:
                if (
                    isinstance(_json["value"][0]["interface_name"], str)
