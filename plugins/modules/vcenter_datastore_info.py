...
options:
  ...
  filter_datastores:
    ...
    - When clients pass a value of this structure as a parameter, the field must contain
      the id of resources returned by M(vcenter_datastore_info).
    elements: str
    type: list
  filter_folders:
    description:
...
async def main():
    required_if = list([["state", "get", ["datastore"], True],])
...
def build_url(params):
    if params.get("datastore"):
        ...
        return (
            ("https://{vcenter_hostname}" "/rest/vcenter/datastore/").format(**params)
            + params["datastore"]
