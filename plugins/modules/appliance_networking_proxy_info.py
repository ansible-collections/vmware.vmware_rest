...
short_description: Handle resource of type appliance_networking_proxy_info
description: Handle resource of type appliance_networking_proxy_info
options:
  protocol:
    description:
...
async def main():
    required_if = list([["state", "get", ["protocol"], True],])
...
def build_url(params):
    if params.get("protocol"):
        ...
        return (
            ("https://{vcenter_hostname}" "/rest/appliance/networking/proxy/").format(
                **params
            )
            + params["protocol"]
...
async def entry_point(module, session):
    ...
    async with session.get(url) as resp:
        ...
        if module.params.get("protocol"):
            ...
            try:
                if (
                    isinstance(_json["value"][0]["protocol"], str)
