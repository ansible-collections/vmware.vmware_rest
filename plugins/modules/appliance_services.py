...
short_description: Handle resource of type appliance_services
description: Handle resource of type appliance_services
options:
  service:
    description:
...
async def main():
    required_if = list(
        [
            ["state", "restart", ["service"], True],
            ["state", "start", ["service"], True],
            ["state", "stop", ["service"], True],
        ]
    )
...
def build_url(params):
    return ("https://{vcenter_hostname}" "/rest/appliance/services").format(**params)
...
async def _restart(params, session):
    ...
    subdevice_type = get_subdevice_type("/rest/appliance/services/{service}/restart")
    if subdevice_type and not params[subdevice_type]:
        _json = await exists(params, session, build_url(params))
        if _json:
            params[subdevice_type] = _json["id"]
    _url = (
        "https://{vcenter_hostname}" "/rest/appliance/services/{service}/restart"
...
async def _start(params, session):
    ...
    subdevice_type = get_subdevice_type("/rest/appliance/services/{service}/start")
    if subdevice_type and not params[subdevice_type]:
        _json = await exists(params, session, build_url(params))
        if _json:
            params[subdevice_type] = _json["id"]
    _url = (
        "https://{vcenter_hostname}" "/rest/appliance/services/{service}/start"
    ).format(**params) + gen_args(params, _in_query_parameters)
    async with session.post(_url, json=payload) as resp:
        try:
            if resp.headers["Content-Type"] == "application/json":
...
async def _stop(params, session):
    ...
    subdevice_type = get_subdevice_type("/rest/appliance/services/{service}/stop")
    if subdevice_type and not params[subdevice_type]:
        _json = await exists(params, session, build_url(params))
        if _json:
            params[subdevice_type] = _json["id"]
    _url = (
        "https://{vcenter_hostname}" "/rest/appliance/services/{service}/stop"
    ).format(**params) + gen_args(params, _in_query_parameters)
    async with session.post(_url, json=payload) as resp:
        try:
            if resp.headers["Content-Type"] == "application/json":
