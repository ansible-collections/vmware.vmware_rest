...
options:
  ...
  subscription_info:
    ...
    - Defines the subscription behavior for this Library. The C(subscription_info)
      ...
      subscription but will not remove existing cached data. Required with I(state=['probe'])
    - 'Valid attributes are:'
...
def prepare_argument_spec():
    ...
    argument_spec["action"] = {"type": "str", "choices": ["sync"]}
...
async def main():
    required_if = list(
        [
            ...
            ["state", "evict", ["library_id"], True],
            ["state", "sync", ["library_id", "~action"], True],
            ["state", "probe", ["subscription_info"], True],
        ]
    )
...
def build_url(params):
    return (
        "https://{vcenter_hostname}" "/rest/com/vmware/content/subscribed-library"
    ).format(**params)
