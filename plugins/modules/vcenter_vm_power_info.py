...
short_description: Get power state information for a virtual machine.
description:
  - Returns power state information for a virtual machine in vCenter.
author:
...
value:
  description:
    - Power state information for the virtual machine.
  returned: On success
  type: dict
  sample:
    state: POWERED_ON
...
POWER_PATH = "/vcenter/vm/{vm}/power"
...
class VmwareRestInfoModule(VmwareRestInfoModuleBase):
    ...
    def get_info(self):
        path = self.build_path(POWER_PATH)
