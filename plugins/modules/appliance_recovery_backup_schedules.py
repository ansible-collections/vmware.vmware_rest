...
DOCUMENTATION = r"""
module: appliance_recovery_backup_schedules
short_description: Handle resource of type appliance_recovery_backup_schedules
description: Handle resource of type appliance_recovery_backup_schedules
options:
  backup_password:
    description:
  ...
  location:
    description:
    - URL of the backup location. Required with I(state=['present'])
    type: str
  location_password:
    description:
  ...
  parts:
    description:
    - List of optional parts to be backed up. Use the {@link appliance.recovery.backup.Parts#list}
...
PAYLOAD_FORMAT = {
    ...
    "delete": {"query": {}, "body": {}, "path": {"schedule": "schedule"}},
    "get": {"query": {}, "body": {}, "path": {"schedule": "schedule"}},
    "create": {
        "query": {},
        "body": {
...
def prepare_argument_spec():
    ...
    argument_spec["comment"] = {"type": "str"}
...
async def main():
    required_if = list(
        [
            ["state", "present", ["location", "schedule"], True],
            ["state", "absent", ["schedule"], True],
            ["state", "run", ["schedule"], True],
        ]
    )
...
def build_url(params):
    return (
        "https://{vcenter_hostname}" "/rest/appliance/recovery/backup/schedules"
    ).format(**params)
...
async def _create(params, session):
    ...
    if params["schedule"]:
        _json = await get_device_info(session, build_url(params), params["schedule"])
    else:
        _json = await exists(params, session, build_url(params), ["schedule"])
    if _json:
        if "_update" in globals():
            params["schedule"] = _json["id"]
...
async def _run(params, session):
    _in_query_parameters = PAYLOAD_FORMAT["run"]["query"].keys()
    payload = prepare_payload(params, PAYLOAD_FORMAT["run"])
    subdevice_type = get_subdevice_type(
    ...
    async with session.post(_url, json=payload) as resp:
        try:
            ...
            _json = {}
        return await update_changed_flag(_json, resp.status, "run")
