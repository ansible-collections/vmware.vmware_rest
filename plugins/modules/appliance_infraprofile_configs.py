...
short_description: Handle resource of type appliance_infraprofile_configs
description: Handle resource of type appliance_infraprofile_configs
options:
  config_spec:
    description:
    - The JSON string representing the desired config specification. Required with
      I(state=['import_profile'])
    type: str
  description:
    description:
...
async def main():
    required_if = list([["state", "import_profile", ["config_spec"], True],])
...
async def _export(params, session):
    ...
    _url = (
        "https://{vcenter_hostname}" "/api/appliance/infraprofile/configs?action=export"
