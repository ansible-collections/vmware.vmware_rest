...
short_description: Retrieves virtual parallel port information for a virtual machine.
description:
  - Returns information about virtual parallel ports belonging to a virtual machine.
  - When I(port) is specified, returns detailed information about that parallel port.
  - When I(label) is specified, returns information for parallel ports with a matching
    device label.
  - When I(port) and I(label) are omitted, lists all parallel ports and returns detailed
...
options:
  ...
  port:
    description:
      - Identifier of the virtual parallel port to retrieve.
      - Must be an identifier (MOID) for a
        C(com.vmware.vcenter.vm.hardware.ParallelPort) resource.
    type: str
  label:
    description:
      - Device label used to filter parallel ports.
      - When a single parallel port matches, the module also returns its MOID in I(id).
    type: str
...
EXAMPLES = r"""
- name: List parallel ports for a virtual machine
  vmware.vmware_rest.vcenter_vm_hardware_parallel_info:
    vm: vm-1001
  register: vm_parallel_ports

- name: Get information about a specific parallel port
  vmware.vmware_rest.vcenter_vm_hardware_parallel_info:
    vm: vm-1001
    port: "4000"
  register: vm_parallel_port_info

- name: Get information about a parallel port by label
  vmware.vmware_rest.vcenter_vm_hardware_parallel_info:
    vm: vm-1001
    label: Parallel port 1
  register: vm_parallel_port_by_label
"""
...
id:
  description:
    ...
    - Must be an identifier (MOID) for a
      C(com.vmware.vcenter.vm.hardware.ParallelPort) resource.
    - Returned when I(label) matches exactly one parallel port.
  returned: When I(label) matches exactly one parallel port
  type: str
  sample: "4000"
...
value:
  description:
    - Virtual parallel port information.
    - Returns a list of parallel port dictionaries when listing or filtering parallel
      ports.
    - Returns a list containing a single parallel port dictionary when I(port) is
      specified.
  returned: On success
  type: list
  elements: dict
