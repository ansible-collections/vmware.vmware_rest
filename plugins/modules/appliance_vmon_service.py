...
short_description: Handle resource of type appliance_vmon_service
description: Handle resource of type appliance_vmon_service
options:
  service:
    description:
...
async def _restart(params, session):
    ...
    if subdevice_type and not params[subdevice_type]:
        ...
        if _json:
            params[subdevice_type] = _json["id"]
    _url = (
        "https://{vcenter_hostname}" "/rest/appliance/vmon/service/{service}/restart"
...
async def _start(params, session):
    ...
    if subdevice_type and not params[subdevice_type]:
        ...
        if _json:
            params[subdevice_type] = _json["id"]
    _url = (
        "https://{vcenter_hostname}" "/rest/appliance/vmon/service/{service}/start"
    ).format(**params) + gen_args(params, _in_query_parameters)
    async with session.post(_url, json=payload) as resp:
        try:
            if resp.headers["Content-Type"] == "application/json":
...
async def _stop(params, session):
    ...
    if subdevice_type and not params[subdevice_type]:
        ...
        if _json:
            params[subdevice_type] = _json["id"]
    _url = (
        "https://{vcenter_hostname}" "/rest/appliance/vmon/service/{service}/stop"
    ).format(**params) + gen_args(params, _in_query_parameters)
    async with session.post(_url, json=payload) as resp:
        try:
            if resp.headers["Content-Type"] == "application/json":
