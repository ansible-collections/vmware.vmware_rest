...
short_description: Retrieves swap health for the vCenter Server Appliance.
description:
  - Returns the swap health level of the vCenter Server Appliance.
  - Uses the vSphere REST API endpoint C(GET /appliance/health/swap).
author:
...
- name: Assert swap health is green
...
value:
  description:
    - Swap health level for the vCenter Server Appliance.
...
SWAP_HEALTH_PATH = "/appliance/health/swap"
...
class VmwareRestInfoModule(VmwareRestInfoModuleBase):
    ...
    def get_info(self):
        response = self.client.get(SWAP_HEALTH_PATH)
