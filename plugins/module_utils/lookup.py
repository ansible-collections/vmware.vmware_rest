...
def get_credentials(**options):
    ...
    credentials['vcenter_username'] = options.get('vcenter_username') or os.getenv('VMWARE_USER')
    credentials['vcenter_password'] = options.get('vcenter_password') or os.getenv('VMWARE_PASSWORD')
    credentials['vcenter_validate_certs'] = options.get('vcenter_validate_certs') or os.getenv('VMWARE_VALIDATE_CERTS')
    credentials['vcenter_rest_log_file'] = options.get('vcenter_rest_log_file') or os.getenv('VMWARE_REST_LOG_FILE')
    return credentials
...
class Lookup:
    ...
    def build_url(self, object_type, params):
        ...
        return (f"https://{self._options['vcenter_hostname']}/api/vcenter/{object_type}") + gen_args(params, _in_query_parameters)
    ...
    async def _fetch_result(self, object_path, object_type, filters):
        ...
        _result = await self.recursive_folder_or_rp_moid_search(object_path, object_type, filters)
        if _result:
            if object_path:
                for obj in _result:
    ...
    async def _get_folder_moid(self, object_path, filters):
        ...
        if object_path:
            _result, _object_path = await self._fetch_result(object_path, 'folder', filters)
        ...
        if self._options['_terms'][-1] == '/' and self._options['object_type'] == 'folder':
    ...
    async def _helper_get_resource_pool_moid(self, object_path, filters):
        ...
        if _object_path:
            result, _object_path = await self._fetch_result(_object_path, 'resource_pool', filters)
        ...
        if self._options['_terms'][-1] == '/' and self._options['object_type'] == 'resource_pool':
    ...
    async def look_inside(self, pre_object, filters):
        ...
        if pre_object:
            if (
                (object_type == 'resource_pool' and 'resgroup' in pre_object) or
    ...
    async def _get_subset_moid(self, object_path, filters):
        ...
        if not result:
            ...
            if self._options['object_type'] == 'vm':
                ...
                result, _object_path = await self._helper_get_resource_pool_moid(object_path, filters)
                if result:
                    return result
    ...
    async def recursive_folder_or_rp_moid_search(self, object_path, object_type, filters,
                                                 parent_object=None, objects_moid=None, result=None):
        if result is None:
            # GET MoID of all the objects specified in the path
            result = []
            ...
            return await self.recursive_folder_or_rp_moid_search(object_path, object_type, filters,
                                                                 objects_moid=objects_moid, result=result)
        parent_key = f'parent_{object_type}s'
        filters[parent_key] = ''

        if (
            objects_moid and object_path and objects_moid[0]['name'] == object_path[0]
        ):
            ...
            if parent_object is not None:
                ...
                [result.append(elem[0]) for elem in _result if elem if _result and elem[0] not in result]
            else:
                _result = await self._helper_fetch(object_type, filters)
                if not _result:
                    return ''
        ...
        if (
            not object_path or
            (objects_moid and len(objects_moid) == 0)
        ):
